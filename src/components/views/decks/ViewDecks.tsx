import useGunSelector from "../../../hooks/useGunSelector";
...
export default function ViewDecks() {
  ...
  const decksIndex = useGunSelector<GunUser["decksIndex"]>(decksIndexRef);
    ...
    <div>
      {decksIndex &&
        ...
        Object.keys(decksIndex).map((deckId) => {
          ...
          return <div key={deckId}>{decks[deckKey].name}</div>;
        })}
    </div>
  );
}
